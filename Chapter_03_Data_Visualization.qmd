...
```{r loading packages}
pacman::p_load("tidyverse", "babynames", "sf", "rnaturalearth", "forcats" ,"tmap")
```
...
## 2. Distributions: Histogram, Density Plots, and Boxplots
...
# Assuming a normal distribution for simplicity, but this can be adjusted as needed
income_with_migration <- rnorm(1000, mean = 40000, sd = 11000)
income_without_migration <- rnorm(1000, mean = 55000, sd = 17500)

# Combine into a data frame
data5 <- data.frame(
  income = c(income_with_migration, income_without_migration),
  migration_status = factor(rep(c("With Migration Background", "Without Migration Background"), each = 1000))
)
```

```{r}
# Create boxplot
ggplot(data5, aes(x = migration_status, 
                 y = income, fill = migration_status)) +
geom_boxplot()  
```
...
```{r}
# Create boxplot
ggplot(data5, aes(x = migration_status, y = income, fill = migration_status)) +
  geom_boxplot(alpha = 0.5, width = 0.5) +
  scale_fill_manual(values = c("#acf6c8", "#D1BC8A")) +
  labs(
    title = "Comparison of Income Distribution by Migration Background",
...
### 3.3 Grouped and Stacked Barplots
...
### 5.2 Scatter Plots with multiple Groups
...
## 6. Making Plots with `facet_wrap()` and `facet_grid()`
...
### 7.1 Combining different types of Graphs
...
### 7.2 Distributions: Ridgeline Chart and Violin Chart
...
### 7.3 Ranking: Lollipop Charts and Radar Charts

#### 7.3.1 Lollipop Charts
