...
Hello, this is an introductory course to the central concepts of statistical analysis and hypothesis testing in R! The goal of this course is to provide you with the basic ideas of statistical analysis and hypothesis testing. For me, it is important that you get an intuition about what is going on rather than terrorizing you with complicated math. Although, I cannot and will not leave out central formulas, you will be fine with the maths. I wish you fun and please do not hesitate to help making that course better.

```{r loading packages}
pacman::p_load("tidyverse", "ggpubr", "gapminder", "kableExtra", 
...
### 2.2 Estimating the Ordinary Least Squares Estimator
...
# Introduce an interaction effect
interaction_effect <- 1.25 # Choose a value that creates a significant interaction
y <- target_correlation * x + sqrt(1 - target_correlation^2) * rnorm(n, sd = 1) + 
    interaction_effect * x * as.numeric(categorical_variable)
...
#Let us first get the cpvariance
...
### 2.3 Predictions with Linear Regression
...
df$y_hat <- -0.33 + 0.68*df$x #First, we calculate the predictions for y

head(df) #Checking 

#We could also do it automatically via the predict() function
automated_predictions <- predict(model1) #Just passing the model to the function 
```
...
head(df) #Checking, you will see the values are differing, that is because of rounding
```
...
ggplot(df, aes(x, residuals_auto)) + 
  ...
  scale_y_continuous("Residuals", seq(-1.5, 1.5, 0.5), 
                     limits = c(-1.5, 1.5)) +
...
#### 3.1.2 Homoskedasticity and Heteroskedasticity
...
```{r homo and hetero plot, warnings = FALSE}
...
y_homo <- 2 * x + e #homoskedastic data 
y_hetero <- 2 * x + e*x^2 #heteroskedastic data 
...
homoskedastic_plot <- ggplot(df_homo_hetero, aes(x = x, y = y_homo)) +
  ...
  scale_y_continuous("Y", seq(-0.5, 3, 0.5), limits = c(-0.5, 3)) +
...
r_squared #0.878 that is approxiametly the same value as we calculated above, again it is not exact, due to rounding

summary(model1)$r.squared #Our value is approxiametly the same value as we calculated above, again it is not exact, due to rounding
```

### 3.3 Influential Outliers
...
#### 4.1.2 Standard Error of the Estimate
...
plotly:: ggplotly(ggplot(t_value_data, aes(x = x, y = density)) + 
  ...
  geom_segment(x = -1.082653, 
               ...
               linetype = "dashed", 
               linewidth = 0.2) +
    geom_point(aes(x = -1.082653, y = dt(-1.082653, df = 28), 
                   color = "pink")) +
  scale_x_continuous("X", seq(-5,5,1), limits = c(-5,5)) +
  theme_classic() +
...
print(p_value_1) #0.28
print(p_value_2) #2.94e-14
```
...
ggplot(data = CIs) +
  ...
  scale_x_continuous(breaks = c(seq(15, 30, by = 1))) # change values on x-axis
...
lm(y ~ x + categorical_variable, data = df) %>% summary()
```
...
### 5.1 Model Fit: Adjusted R-squared
...
summary(multivariate_model)$adj.r.squared #Extract Adjusted R-squared

#By Hand

adj_r_squared <- 1 - (((1-summary(multivariate_model)$r.squared) * (nrow(df) - 1))/(nrow(df) - 2 - 1))

print(adj_r_squared) #0.88
```
...
#Let us display both models next to each other.EDIT: I created this function specifically, the code for the function is at the top.   
...
#### 5.3.1 Testing Correlations to each other
...
#### 5.3.2 Variance of Inflation (VIF)
...
Interactions effects can be implemented in R, by simply writing it explicitly into the function, either with an asterisks `*` or a double point `:`, let us have a look it:

```{r model with interaction effect}

model_interaction <- lm(y ~ x + categorical_variable + categorical_variable*x, data = df) 
...
As you can see a coefficient is displayed, but the interpretation is a bit messy. With a one unit increase in x for group A,, the dependent variable y decreases by -0,47, in comparison to a one-unit increase in x for group B on average, holding all else constant.

This interpretation is not intuitive, and a mess. We only have two groups, but what if we interact a variable with a categorical variable with 5 categories? Or with another 10-point scale variable? Visualization can help us to get a better intuition.

```{r plotting interaction effect}
ggplot(df, aes(x = x, y = y, color = categorical_variable)) + 
  geom_smooth(method = "lm") +
  scale_color_manual(values = c("#ED2939", "#0A6522")) +
  scale_x_continuous(
    breaks = seq(0,10, 1)) +
  labs(x = "X", 
       y = "Y", 
...
On the x-axis, we have our independent variable and on the y-axis we do have our dependent variable. So far, so normal. But we see two lines: One line for all observations in category 0 and one line for all observations 1. That is exactly what an interaction does. For every point in our independent variable a coefficient is computed in the category 0, and one line for every observation in the category 1.
...
1.  **Direction of the Lines:**
    -   Both lines (for group 0 and group 1) are increasing, indicating that more study hours lead to higher exam scores for both groups.
2.  **Effect Comparison**:
    -   The slope for group 1 (students who attended the preparatory course) is steeper compared to group 0 (students who did not attend the preparatory course).

    -   This means that for students who attended the preparatory course, each additional hour of study has a larger positive impact on their exam scores compared to those who did not attend the course.
