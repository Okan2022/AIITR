...
```{r, eval = FALSE}
library(pacman) #You run the code and voila you can use the package
```
...
```{r}
pacman::p_load("tidyverse", "haven", "psych", "WDI", "gapminder") 
```

## 3. The Data we will work with: The European Social Survey (ESS)
...
#### 4.1.2 Working Directories and R-Projects
...
#### 5.1.1 Filtering for only one condition
...
```{r}
d1 <- ess %>% 
  filter(cntry == "HU") #filtering for cases only in Hungary.

head(d1) #Lets have a look if everything worked out 
...
#### 5.1.2 Filtering for multiple condition
...
#### 5.2.1 Selecting and deleting single Rows

-   We only have to pass their column names to `select()`. That's it.

```{r}
d1 <- ess %>%
   select(year, cntry, happy, agea, gndr, eisced) #Let's get the for us relevant variables

head(d1) #always check
```
...
```{r}
d2 <- d1 %>% 
  select(-agea) #We delete columns by simply putting a comma before it

head(d2)
```

#### 5.2.3 Combining `select()` with the `filter()` function
...
#### 5.3.1 Arranging in ascending order
...
#### 5.3.2 Arranging in descending order
...
#### 5.4.1 Renaming Variables: `rename()`
...
#### **5.4.2 Relocating Variables: `relocate()`**
...
d2 <- ess %>%
  ...
  rename(country = cntry, 
         ...
         education = eisced, 
         female = gndr) %>%
  relocate(country, .after = age) #after with a point in front of it puts the variable after age
...
d3 <- ess %>%
  ...
  rename(country = cntry, 
         ...
         education = eisced, 
         female = gndr) %>%
  relocate(country, .before = age) #after with a point in front of it puts the variable after age
 #before with a point in front of it puts the variable before age
...
table(ess$happy) # Check the variables

d1 <- ess %>% 
  mutate(
    gndr_fac = as.factor(gndr), #always check the class 
    happy_cat = recode(happy,
                    `0` = 0,
                    `1` = 0,
                    `2` = 0,
                    `3` = 0,
...
#Do not get confused with the brackets and the equal signs, I always do...
...
#### 5.5.4 Handling Missing Values/Incomplete Data
...
d1 <- ess %>%
  ...
  select(year, cntry, netusoft, agea, eisced, gndr, happy) %>% #selecting only relevant variables 
  arrange(desc(agea)) %>% #arranging them 
  rename(
...
### 5.6 The `group_by()` and `summarize()` functions
...
#### 5.6.1 With one grouping variable and one metric
...
#### 5.6.2 With more grouping variables and metrics
...
#### 6.1 Introduction to merging with `dplyr` and preparing data
...
```{r}
indicators = c("NY.GDP.PCAP.CD", "TX.VAL.FUEL.ZS.UN", 
               "EN.ATM.CO2E.KT")
```
...
head(d1) #The skimr package also helps

```

#### 6.2 `left_join()` and `right_join()` with one identifier
...
#### 6.3 `left_join()` and `right_join()` with two identifiers
